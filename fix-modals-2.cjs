...
// Replace CustomerModalBackend
code = code.replace(/function CustomerModalBackend\(\{ onClose, onCreate \}\) \{[\s\S]*?\}\n\n/g, `function CustomerModalBackend({ onClose, onCreate }) {
...
// Replace SupplierModalBackend
code = code.replace(/function SupplierModalBackend\(\{ onClose, onCreate \}\) \{[\s\S]*?\}\n\n/g, `function SupplierModalBackend({ onClose, onCreate }) {
  const [form, setForm] = useState({ name: '', phone: '', email: '', address: '' });
  const [submitting, setSubmitting] = useState(false);
  const handleSubmit = async (e) => {
    e.preventDefault();
...
// Replace ExpenseModalBackend
code = code.replace(/function ExpenseModalBackend\(\{ onClose, onCreate \}\) \{[\s\S]*?\}\n\n/g, `function ExpenseModalBackend({ onClose, onCreate }) {
  ...
  return <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
    <div className="bg-white rounded-[24px] w-full max-w-md shadow-2xl max-h-[90vh] flex flex-col">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
        <div><label className="text-xs font-semibold uppercase text-slate-500 tracking-wider">Date</label><input type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})} className="mt-1 w-full h-12 px-4 border-2 border-slate-100 rounded-xl focus:border-violet-500 focus:outline-none bg-white" /></div>
