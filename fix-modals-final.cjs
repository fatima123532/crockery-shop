...
// Replace CustomerModalBackend safely using regex that bounds on the next function definition
code = code.replace(/function CustomerModalBackend\(\{[\s\S]*?(?=function SupplierModalBackend|function ExpenseModalBackend|function [A-Z])/g, `function CustomerModalBackend({ onClose, onCreate }) {
...
code = code.replace(/function SupplierModalBackend\(\{[\s\S]*?(?=function ExpenseModalBackend|function [A-Z])/g, `function SupplierModalBackend({ onClose, onCreate }) {
  const [form, setForm] = useState({ name: '', phone: '', email: '', address: '' });
  const [submitting, setSubmitting] = useState(false);
  const handleSubmit = async (e) => {
    e.preventDefault();
...
code = code.replace(/function ExpenseModalBackend\(\{[\s\S]*?(?=function [A-Z])/g, `function ExpenseModalBackend({ onClose, onCreate }) {
...
// Clean up "********" placeholders everywhere else
